...
#%% Load data
# path_dir = r'C:\temp_asn\[p.legacy_ps][s.chicken_smallscan][06-11-2021_18-11-27]'
path_dir=r'I:\ps-closer-look-data\[p.S3B_11_25_19][s._S_loc3][11-25-2019_12-38-31]'
...
if b_change_editsetting:
    ...
    data.reconstructionSettings['depthIndex'] = [1100,1800] # [1100,1500]#(phantom) 
...
processed = processor.processFrameRange(data, procState='struct+ps+stokes+oa+iquv+null', #procState='struct+ps+hsv+oa',#, procAll=True, writeState=True)
                            procAll=False, startFrame=1000, endFrame=1000, 
                            writeState=True,
...
#------------------------------------------
if True:
    path_dir = r'D:\OFDIData\user.Ilyas\[p.S3B_11_25_19][s._S_loc3][11-25-2019_12-38-31]'
#------------------------------------------batch processing or single selection
    ...
    if os.path.isdir(path_dir):
        ...
        processor.processFrameRange(data, procState='struct+ps+stokes+oa',#, procAll=True, writeState=True)
                                    procAll=False, startFrame=1101, endFrame=1101, 
                                    writeState=True)
    else:
